...
void giveInfo(float available_memory, int num_of_pages, int num_of_free_pages, float size_of_page){
	...
	std::cout << "Кол-во страниц: " << num_of_pages << "\n3.Показать таблицу";
...
void do_smth(data *a, int prikol){
	int i = 0 + rand() % (prikol-1) ;
	(a+i)->appeal = (a+i)->appeal + (0 + rand() % 10);
}
...
int main(){
	...
	float size_of_page = (float)available_memory / num_of_pages;
